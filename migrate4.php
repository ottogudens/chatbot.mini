...
foreach ($models_to_fix as $old_model) {
    ...
    if (mysqli_query($conn, "UPDATE assistants SET gemini_model='gemini-2.0-flash' WHERE gemini_model='$safe'")) {
        $n = mysqli_affected_rows($conn);
        if ($n > 0)
            echo "OK: $n asistentes actualizados de '$old_model' a 'gemini-2.0-flash'.\n";
    }
}

// Fix the column DEFAULT back to gemini-2.0-flash
if (mysqli_query($conn, "ALTER TABLE assistants MODIFY COLUMN gemini_model VARCHAR(50) NOT NULL DEFAULT 'gemini-2.0-flash'")) {
    echo "OK: DEFAULT de gemini_model corregido a 'gemini-2.0-flash'.\n";
} else {
